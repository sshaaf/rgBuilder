...
```bash
rg-build -r example/coolstore-weblogic -f json metrics --pagerank
```
...
```bash
rg-build -r example/coolstore-weblogic -f json metrics --pagerank --iterations 50
```
...
```bash
rg-build -r example/coolstore-weblogic -f json metrics --betweenness
```
...
```bash
rg-build -r example/coolstore-weblogic -f json metrics --communities
```
...
- **`modularity: 0.3077`** -- a modularity score of ~0.31. Values above 0.3 indicate meaningful community structure; values above 0.5 indicate strong modularity. The CoolStore monolith has moderate modularity, consistent with its nature as a single-deployment application.
...
```bash
rg-build -r example/coolstore-weblogic -f json metrics --pagerank --betweenness --communities
```

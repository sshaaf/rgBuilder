...
**Profiling:** `RUST_LOG=profile=info rgbuilder discover . -v` → `[profile] centrality sub-phase` lines.

**CLI precision:** `rgbuilder metrics --pagerank --iterations N` still honors explicit iteration counts on demand.
...
rgbuilder -f json metrics --betweenness
rgbuilder -f json metrics --communities
```

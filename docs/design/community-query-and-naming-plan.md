...
### Acceptance
```bash
rgbuilder -r "$REPO" -f json gql 'MATCH (c:Community) RETURN c'
...
| **3c LLM / agent label** | Opt-in `rgbuilder communities label` (or discover flag) with member sample → short title | CI/offline; never required for core |

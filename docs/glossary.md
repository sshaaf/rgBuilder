...
| **Discover** | Index a repository into `.rgbuilder/` (graph snapshot + analytics). |
| **Fusion** | Late re-ranking of semantic hits with graph signals (blast, PageRank, sketches). |
| **GQL** | rgBuilder graph query language (`MATCH` / macros) over the knowledge graph. |
...
| **L_proc** | Per-function procedural layer (CFG/PDG archive under `.rgbuilder/analysis/`). |
...
| **Semantic index** | Opt-in function embedding store (`.rgbuilder/semantic_index.bin`). |

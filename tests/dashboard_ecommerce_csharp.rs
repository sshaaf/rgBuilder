...
//!
//! Repo path: `/Users/sshaaf/git/rust/rgbuilder-tests/ecommerce-csharp`
//! (override: `RGBUILDER_CSHARP_REPO`).
...
fn discover_all_writes_csharp_cfg_dashboard_bundle() {
    ...
    if !repo.is_dir() {
        eprintln!(
            "skip: csharp test repo not found at {} (set RGBUILDER_CSHARP_REPO)",
            repo.display()
        );
        return;
    }

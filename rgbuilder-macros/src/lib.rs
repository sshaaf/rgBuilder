//! Procedural macros for rgBuilder language plugins (Phase 7.2)
...
pub fn derive_language_plugin(input: TokenStream) -> TokenStream {
    ...
    let new_impl = if let Some(ref grammar) = grammar_crate {
        if has_parser_field {
            quote! {
                pub fn new() -> Result<Self, rgbuilder_plugin_api::Error> {
                    let mut parser = tree_sitter::Parser::new();
                    parser
                        .set_language(&#grammar::LANGUAGE.into())
                        .map_err(|e| rgbuilder_plugin_api::Error::PluginError(

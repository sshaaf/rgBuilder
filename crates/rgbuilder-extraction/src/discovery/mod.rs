...
mod tests {
    ...
    fn discoverer() -> FileDiscoverer {
        FileDiscoverer::new(Arc::new(rgbuilder_languages::default_registry()))
    }
    ...
    fn test_custom_exclude_patterns() {
        ...
        let files =
            FileDiscoverer::with_config(Arc::new(rgbuilder_languages::default_registry()), config)
                .discover(root)
                .unwrap();
    ...
    fn test_discover_missing_path() {
        let result = discoverer().discover(Path::new("/nonexistent/rgbuilder-test-path"));

...
use rgbuilder_graph::code_graph::{CodeGraph, GRAPH_DIR};
...
pub struct FileHashMetadata {
    /// rgBuilder version that created this metadata
    pub version: String,
...
impl FileTracker {
    ...
    /// Load persisted hash metadata from `.rgbuilder/file_hashes.json`.
    ...
    if let Ok(nodes) = graph.backend().all_nodes() {
        for node in nodes {
            let file = if let Some(path) = node.file_path.as_deref() {
                Some(path)
            } else if matches!(node.node_type, rgbuilder_graph::schema::NodeType::File) {

//! Error types for rgBuilder
//!
//! This module defines all error types used throughout the rgBuilder system.
//! We use `thiserror` for ergonomic error definitions with automatic trait implementations.

use std::path::PathBuf;
use thiserror::Error;

/// Main error type for rgBuilder operations
#[derive(Error, Debug)]
pub enum Error {
...
/// Result type alias for rgBuilder operations
...
impl From<rgbuilder_plugin_api::Error> for Error {
    fn from(err: rgbuilder_plugin_api::Error) -> Self {
        match err {
            rgbuilder_plugin_api::Error::ParseError {
                file,
                line,
                message,
            } => Error::ParseError {
            ...
            rgbuilder_plugin_api::Error::PluginError(message) => Error::PluginError(message),
            rgbuilder_plugin_api::Error::UnsupportedLanguage(language) => {

...
use rgbuilder_analysis::{FieldWriteIndex, FieldWriteKind};
...
mod tests {
    use super::*;
    use rgbuilder_analysis::{FieldWrite, FieldWriteIndex, FieldWriteKind};
    ...
    fn exports_writes_and_types() {
        let tmp = tempdir().unwrap();
        let analysis = tmp.path().join(".rgbuilder").join("analysis");

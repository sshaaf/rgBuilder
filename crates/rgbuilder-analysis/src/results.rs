...
impl StructuralSketchTable {
    ...
    pub fn with_capacity(node_count: usize) -> Self {
        Self {
            token_blooms: vec![0; node_count * rgbuilder_graph::TOKEN_BLOOM_WORDS],
        }
    }

    /// Write one bloom sketch for `compact_id`.
    pub fn set_bloom(&mut self, compact_id: CompactId, bloom: rgbuilder_graph::TokenBloom) {
        let offset = compact_id as usize * rgbuilder_graph::TOKEN_BLOOM_WORDS;
        if offset + rgbuilder_graph::TOKEN_BLOOM_WORDS <= self.token_blooms.len() {
            self.token_blooms[offset..offset + rgbuilder_graph::TOKEN_BLOOM_WORDS]
                .copy_from_slice(&bloom);
        }
    }

    /// Read the bloom sketch for `compact_id`.
    pub fn bloom(&self, compact_id: CompactId) -> Option<rgbuilder_graph::TokenBloom> {
        let offset = compact_id as usize * rgbuilder_graph::TOKEN_BLOOM_WORDS;
        let words = self
            .token_blooms
            .get(offset..offset + rgbuilder_graph::TOKEN_BLOOM_WORDS)?;
    ...
    pub fn satisfies_keyword_and(&self, compact_id: CompactId, keywords: &[String]) -> bool {
        self.bloom(compact_id)
            .is_some_and(|bloom| rgbuilder_graph::satisfies_keyword_and(keywords, &bloom))
    }
    ...
    pub fn keyword_overlap(&self, compact_id: CompactId, keywords: &[String]) -> f64 {
        self.bloom(compact_id)
            .map(|bloom| rgbuilder_graph::keyword_overlap_score(keywords, &bloom))
            .unwrap_or(0.0)
    }
}
...
mod tests {
    ...
    fn profile_save_linux_artifact() {
        ...
        let path = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../example/linux/.rgbuilder/analysis_results.bin");
        if !path.is_file() {
            eprintln!(
        ...
        results
            .save(&Path::new("/tmp/rgbuilder-analysis_results-resave.bin"))
            .unwrap();
    }
}

...
impl Ruleset {
    ...
    pub fn from_file(path: &std::path::Path) -> rgbuilder_error::Result<Self> {
        let content = std::fs::read_to_string(path)?;
        Self::from_json(&content)
    }

    /// Parse a ruleset from JSON.
    pub fn from_json(json: &str) -> rgbuilder_error::Result<Self> {
        let mut ruleset: Ruleset = serde_json::from_str(json)
            .map_err(|e| rgbuilder_error::Error::SerdeError(e.to_string()))?;

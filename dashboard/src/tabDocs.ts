...
export const TAB_DOCS: Record<TabId, TabDocContent> = {
  ...
  search: {
    ...
    description:
      "Uses the bundled code-daemon embedder and Hamming nearest-neighbor index built by `rgbuilder semantic index`. Late fusion re-ranks candidates with blast radius, centrality, name overlap, and eager token-bloom sketches. Requires `rgbuilder serve` (the query API is server-side).",
    benefits: [
    ...
    usage: [
      "Run `rgbuilder semantic index` once per repo (after discover).",
      "Start the dashboard with `rgbuilder serve --open`.",

...
type SceneApi = SceneNavApi & {
  setLod: (lod: LodLevel, communityId: number | null) => void;
...
export function CosmosScene({
  ...
  useEffect(() => {
    ...
    const buildL2Meshes = (neighborhood: L2Neighborhood) => {
      ...
      const placements = layoutSubgraphNodes3d(
        neighborhood.payload.nodes,
        neighborhood.anchor,
        neighborhood.packageId,
      );
    ...
    const setLod = (nextLod: LodLevel, communityId: number | null) => {
      for (const mesh of packageMeshes.values()) {
        mesh.visible = false;
      }
      if (nextLod >= 1 && communityId != null) {
        for (const pkg of layout.packages) {
          if (pkg.community_id !== communityId) continue;
          packageMeshes.get(pkg.id)!.visible = true;
        }
      }
      if (nextLod < 2) clearL2Meshes();
    };

    setLod(lodRef.current, communityRef.current);
    ...
    const animate = () => {
      ...
      } else if (lodRef.current === 0) {
        communityGroup.rotation.y = frame * 0.00012;
      }

      if (lodRef.current >= 2 && l2Built && l2Anchor) {
        const show = shouldShowL2Nodes(
  ...
  useEffect(() => {
    apiRef.current?.setLod(lod, selectedCommunityId);
  }, [lod, selectedCommunityId]);
